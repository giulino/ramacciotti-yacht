...
import { i as isValidEmail, s as saveEncryptedInquiry } from "../../../../chunks/form-security.js";
async function POST({ request, getClientAddress }) {
  ...
  if (name.length > 120 || email.length > 160 || phonePrefix.length > 8 || phone.length > 40 || message.length > 3e3) {
  ...
  try {
    await saveEncryptedInquiry(payload, private_env.FORM_ENCRYPTION_KEY ?? "");
  } catch (error) {
    console.error("Inquiry storage failed", error);
    return json(
      {
        error: "Form processing is not configured yet. Set FORM_ENCRYPTION_KEY on the server to enable secure submissions."
      },
      { status: 503 }
    );
  }
  const webhook = private_env.EMAIL_WEBHOOK_URL;
  if (webhook) {
    try {
      await fetch(webhook, {
        method: "POST",
